                  ...
                  <div class="columns">
                    <div class="column is-narrow">
                      <figure class="image is-4by2 is-128x128">
                        <img
                          :src="
                            course.image || 'https://via.placeholder.com/128'
                          "
                        />
                      </figure>
                    </div>
                    <div class="column">
                      ...
                      <span class="tag" :class="courseStatus(course.status)">
                        {{ course.status }}
                      </span>
                    </div>
...
export default {
  ...
  async fetch({ store }) {
    await store.dispatch('instructor/courses/fetchInstructorCourses')
  },
  computed: {
    ...mapGetters({ courses: 'instructor/courses/getCourses' })
  },
  methods: {
    courseStatus(status) {
      if (!status) return ''
